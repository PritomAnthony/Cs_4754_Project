...
app.get('/generateBill/:bookingNumber', (req, res) => {
  const bookingNumber = req.params.bookingNumber;
  if (!bookingNumber) {
    return res.status(400).send({ message: 'Booking Number is required.' });
  }

  connection.query( //call stored procedure
    'CALL generateBill(?, @total_fees);', 
    [bookingNumber],
    (err, results) => {
      if (err) {
        console.error('Error executing query:', err);
        return res.status(500).send({ message: 'Server error while generating bill.', error: err });
      }

      connection.query( //total fees query
        'SELECT @total_fees AS totalFees;',
        (err, totalFeeResults) => {
          if (err) {
            console.error('Error fetching total fees:', err);
            return res.status(500).send({ message: 'Error fetching total fees.', error: err });
          }

          // result set contains the bill details (room and food costs)
          const billDetails = results[0];

          // total_fees result
          const totalFees = totalFeeResults[0].totalFees;

          // format
          const formattedBillDetails = billDetails.map(row => ({
            Label: row.Label,
            COST: row.COST
          }));
